...
class SettlementReport extends Resource
{
     ...
     *
     * Pinned to v3 — Flip only documents this endpoint on v3.
     *
     * Returns a `request_id` you can poll with `checkStatus()`.
     *
     * Endpoint: `POST /v3/settlement-report/generate`.
     *
    ...
    public function generate(string $startDate, string $endDate): array
    {
        return $this->client->postJson(
            $this->path('settlement-report/generate', 'v3'),
            [
     ...
     *
     * Pinned to v3 — see `generate()`.
     *
     * Endpoint: `GET /v3/settlement/settlement-report/check-status`.
     *
    ...
    public function checkStatus(?string $requestId = null, ?string $disbursementId = null): array
    {
        return $this->client->get(
            $this->path('settlement/settlement-report/check-status', 'v3'),
